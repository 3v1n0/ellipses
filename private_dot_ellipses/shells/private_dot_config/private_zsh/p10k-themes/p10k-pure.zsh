# Generated by Powerlevel10k configuration wizard on 2020-05-22 at 21:00 CEST.
# Based on romkatv/powerlevel10k/config/p10k-pure.zsh, checksum 00682.
# Wizard options: flat, large icons, pure, 24h time, 2 lines, sparse,
...
() {
  ...
  # FIXME: Change this when running on TTYs
  ...
  if [ -n "$SCHROOT_ALIAS_NAME" ]; then
    ...
    if [[ "$HOST" == "tricky" ]]; then
      host_color=208
    elif [[ "$HOST" == "t460s" ]]; then
      host_color=184
    elif [[ "$HOST" == "ubuntu-vmware" ]]; then
      host_color=75
    fi
